...
--Вспомогательная процедура для перевода временного интервала в минуты
...
begin
	select @interval =
			--количество минут в годе (високосные годы в этой версии не учитываются)
...
begin
	...
	--имя таблицы (номер оборудования) может меняться, используем динамический запрос
	--для прохода по выборке и вычислений интервалов используем курсор
	set @query =						
	...
	--подсчитываем процент простоя от общего времени
	set @downtime_percent = cast(cast(@downtime as decimal) / @total_interval * 100 as decimal)	
end;

--время простоя из-за останова оборудования, процедура построена аналогично предыдущей, за исключением условия выборки
...
--подсчитываем время рабочего состояния
